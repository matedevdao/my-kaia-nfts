...
import { AppCompConfig, Button, PromptDialog, } from "@commonmodule/app-components";
...
export default class NFTView extends View {
    ...
    async renderNFTDisplay(nftData) {
        this.container.append(el("header", nftData.holder === KaiaWalletLoginManager.getLoggedInAddress()
            ...
            : undefined, nftData.holder === KaiaWalletLoginManager.getLoggedInAddress() &&
            MDDModuleConfig.isEditableNFTCollection(nftData.collection)
            ? new Button({
                title: "정보 수정",
                onPress: () => Router.go(`/${nftData.collection}/${nftData.id}/edit`, nftData),
            })
